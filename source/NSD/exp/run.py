...
from nsd_utils.heterophilic import get_dataset, get_fixed_splits
...
def run_my_data(args, dataset, model_cls, fold):
    data = dataset[0]
    # data = get_fixed_splits(data, args['dataset'], fold)
    data = data.to(args['device'])
...
if __name__ == '__main__':
    ...
    print(args)
    wandb.init(project="sheafs", config=vars(args), entity="codecre8or")

    if args.dataset in ["tadpole", "abide", "ukbbcardiac", "covid", "ukbbage"]:
        test_acc, best_val_acc, keep_running = run_my_data(wandb.config, dataset, model_cls, 0)
